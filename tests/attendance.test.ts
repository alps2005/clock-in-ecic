...
const context: AppContext = { profile: { id:'p',auth_user_id:'a',full_name:'Fixture',role:'teacher',active:true,cedula:'0000000001' },server_time:'2026-09-14T11:00:00Z',school_date:'2026-09-14',working_day:true,events:[],policy:{id:'policy',timezone:'America/Guayaquil',weekdays:[1,2,3,4,5],entry_opens:'06:00:00',entry_closes:'06:45:00',exit_opens:'12:45:00',exit_closes:'13:30:00'} }
test('UI reflects exact scanner windows, late justification and missing exit', () => {
  ...
  assert.equal(attendanceAction(context,new Date('2026-09-14T11:45:00Z')),'entry')
  assert.equal(attendanceAction(context,new Date('2026-09-14T11:45:00.001Z')),'late_entry')
  const entered = {...context,events:[{sequence_no:1} as AttendanceEvent]}
  assert.equal(attendanceAction(entered,new Date('2026-09-14T17:44:59.999Z')),'before_exit')
  assert.equal(attendanceAction(entered,new Date('2026-09-14T17:45:00Z')),'exit')
