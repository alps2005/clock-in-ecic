import { roleLabels, staffRoles } from '../../lib/roles'
...
export function TeacherEditor({ editor, schoolDate, close, saved }: { editor: Editor; schoolDate: string; close: () => void; saved: () => void }) {
  ...
  const [role, setRole] = useState<StaffRole>(teacher?.role ?? 'teacher')
  ...
  async function submit(event: FormEvent) {
    ...
    if (details) Object.assign(input, { role, full_name: name.trim(), cedula, employed_from: from, employed_until: until || null, ...(action === 'update' ? { active } : {}) })
