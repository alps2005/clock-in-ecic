...
export function TeacherPage({ context }: { context: AppContext }) {
  const { teacherId = '' } = useParams()
  return <TeacherDetails key={teacherId} teacherId={teacherId} context={context} />
}

function TeacherDetails({ teacherId, context }: { teacherId: string; context: AppContext }) {
  ...
  const [action, setAction] = useState<Editor['action'] | null>(null)
