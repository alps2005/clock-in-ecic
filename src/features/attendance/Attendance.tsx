...
export function Attendance({ context, refresh }: { context: AppContext; refresh: () => void }) {
  ...
  return <>
    ...
    <div className={`jornada-alert tone-${state.tone === 'neutral' ? 'accent' : state.tone}`} role="status">{state.tone === 'success' ? <CircleCheck size={20} aria-hidden="true" /> : scannerAllowed ? <ScanLine size={20} aria-hidden="true" /> : <Info size={20} aria-hidden="true" />}<div><strong>{state.title}</strong><p>{state.description}</p></div>{state.showHistory && <Link className="button secondary" to="/historial">Ver mi historial</Link>}</div>
    ...
    <section className={`ui-card jornada-schedule${!context.working_day ? ' no-jornada' : ''}`}>
      ...
      <div className="jornada-windows">{[{ title: 'Entrada', Icon: LogIn, start: entryOpens, end: entryCloses, hint: 'Escanea el QR al llegar.', status: state.entry }, { title: 'Salida', Icon: LogOut, start: exitOpens, end: exitCloses, hint: 'Registra el cierre de tu jornada.', status: state.exit }].map(({ title, Icon, start, end, hint, status }) => <div className="jornada-window" key={title}><span className="window-icon"><Icon size={18} aria-hidden="true" /></span><div><h3>{title}</h3><strong className="numeric">{start}–{end}</strong><p>{hint}</p></div><span className={`ui-badge tone-${status.tone}`}>{status.window}</span></div>)}</div>
