...
export async function loadWeeklyRows(today: string, loadPage: (from: string, to: string, page: number) => Promise<Report>) {
  const week = currentSchoolWeek(today)
  const first = await loadPage(week.from, week.to, 0)
  const rows = [...first.rows]
  const pages = Math.ceil(first.totals.expected / first.page_size)
  for (let page = 1; page < pages; page++) {
    const next = await loadPage(week.from, week.to, page)
    rows.push(...next.rows)
  }
  return {
    ...week,
    rows: rows.filter(row => row.school_date >= week.from && row.school_date <= week.to && row.school_date <= today)
      .sort((a, b) => a.school_date.localeCompare(b.school_date) || a.full_name.localeCompare(b.full_name, 'es') || a.teacher_id.localeCompare(b.teacher_id)),
  }
}
