...
function CalendarDialog({ label, value, onChange, close }: { label: string; value: string; onChange: (value: string) => void; close: () => void }) {
  ...
  useEffect(() => { dialog.current?.querySelector<HTMLButtonElement>(`[data-date="${focused}"]`)?.focus() }, [dialog, focused])
  ...
  return <dialog ref={dialog} onKeyDown={trapDialogFocus} className="date-dialog" aria-labelledby="calendar-title" onCancel={event => { event.preventDefault(); dismiss() }}>
    <div className="dialog-heading"><h2 id="calendar-title">{label}: selecciona una fecha</h2><button type="button" className="icon-button" aria-label="Cerrar calendario" onClick={dismiss}><X size={18} /></button></div>
    ...
    <div className="calendar-days" aria-label={monthLabel}>{Array.from({ length: 42 }, (_, i) => {
      ...
      return <button type="button" key={date} data-date={date} className={`${date.slice(0, 7) !== month ? 'other-month' : ''}${date === value ? ' selected' : ''}`} aria-label={displayDate(date)} aria-pressed={date === value} tabIndex={date === focused || (focused.slice(0, 7) !== month && date === first) ? 0 : -1} onClick={() => { onChange(date); dismiss() }} onKeyDown={event => {
