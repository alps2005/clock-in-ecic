export type Profile = { id: string; auth_user_id: string | null; cedula: string; full_name: string; role: 'teacher' | 'admin'; active: boolean }
...
export type ReportRow = { teacher_id: string; full_name: string; cedula: string; school_date: string; entry_at: string | null; exit_at: string | null; justification: string | null; entry_status: 'pending' | 'on_time' | 'late' | 'late_pending' | 'absent'; exit_status: 'pending' | 'registered' | 'missing'; worked_minutes: number | null }
export type Report = { as_of: string; page: number; page_size: number; totals: { expected: number; on_time: number; late: number; absent: number; missing_exit: number; completed: number }; rows: ReportRow[] }
...
export type TeacherMutation = { action: 'create' | 'update' | 'reset-password' | 'disable'; id?: string; full_name?: string; cedula?: string; password?: string; active?: boolean; employed_from?: string; employed_until?: string | null }
